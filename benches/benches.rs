...
use kvs::{thread_pool::*, KvStore, KvsEngine, SledKvsEngine};
...
fn get_random_kv_vec(k_range: usize, value: String, length: u64) -> Vec<(String, String)> {
    ...
    let res: Vec<(String, String)> = (0..length)
        .map(|_| (get_a_string(&mut rand, k_range), value.clone()))
        .collect();
    return res;

    fn get_a_string(rand: &mut ChaChaRng, range: usize) -> String {
        let v: &mut [u8] = { &mut Vec::with_capacity(range) };
        rand.fill_bytes(v);
...
fn write_queued_kvstore(c: &mut Criterion) {
    ...
    c.bench_function_over_inputs(
        ...
        |b, &&cpu_core_num| {
            ...
            let kv_vec: Vec<(String, String)> =
                get_random_kv_vec(KEY_LEN, VALUE.to_string().clone(), KV_LEN);
